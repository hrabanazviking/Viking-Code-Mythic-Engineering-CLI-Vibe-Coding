...
- **H-4 — Two copies of NSE systems, each incomplete.** `systems/` at root has 27 files; `imports/norsesaga/systems/` has only 3 (event_dispatcher, world_dreams, world_will). The inner copy appears to be a stripped subset — but `core/dream_system.py` and `imports/norsesaga/systems/world_dreams.py` both describe "world dreams" with different implementations.
