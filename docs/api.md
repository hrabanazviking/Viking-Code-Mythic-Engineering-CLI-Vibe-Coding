...
- `workflow history`
  - Lists recorded workflow plan saves from `mythic/workflow_history.json`, newest first. `--limit N` caps the returned entries. JSON output includes the resolved `history_path`, `count`, `total`, and full entries with `workflow_id`, `task`, `created_at`, `plan_path`, and `role_sequence`. Dry-run plans never grow the ledger; the file is capped at 50 entries.
...
`plugin` exposes the versioned grimoire registry. `plugin list` reports registry health without importing plugin code, `plugin inspect` imports one registered `module:object` entrypoint and reports declared hooks, and `plugin disable` preserves a plugin record while preventing it from being treated as active. Supported hooks are `before_scan`, `after_scan`, `before_packet`, `after_packet`, `before_verify`, `after_verify`, `before_reflect`, and `after_reflect`. All eight hooks have real emitters in the active command surface (scan, packet create / ingest / workflow plan --packets, verify, reflect). For the operator-facing guide on writing a plugin — hook signatures, payload shapes, the registration ritual, constraints — see [`docs/plugins.md`](plugins.md).

`tui` opens a Textual-based status TUI showing project phase, last verification, latest handoff, and plugin counts with auto-refresh. Keybindings: `q` quit, `r` refresh. Requires the optional `[tui]` extra (`pip install "mythic-vibe-cli[tui]"`); a clear install message is printed if the extra is missing. Cross-platform via Textual (pure Python, MIT).

`slash list` prints the slash-command catalog: `BUILTIN_SLASH_COMMANDS` plus any entries contributed by plugins via an optional `slash_commands()` callable on the plugin class. `--source builtin|extension|prompt|skill|plugin` filters the output. JSON output exposes `builtin` and `contributed` arrays. See [`docs/plugins.md`](plugins.md) §9 for the plugin-side contract and [`docs/runtime.md`](runtime.md) §6 for the catalog and §7 for the source-info provenance.
