...
- `packet diff --latest-workflow` lets `--left` and `--right` additionally accept a bare `step-NN` form that resolves against the workflow id stored in `mythic/workflow_plan.json`. `PKT-...` IDs and `WF-<id>:<step_id>` shorthand continue to work in the same call. JSON output reports the resolved `latest_workflow_id`.
