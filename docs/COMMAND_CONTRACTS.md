...
- Bare `mythic` / `mythic-vibe` opens the interactive companion shell by default. `mythic-vibe shell` remains as an explicit compatibility command for the same surface. The shell startup prints current project/repository, Git branch, selected model, memory status, and knowledge status. It reads command lines from stdin via `input()`, dispatches slash commands and known bare commands to `app.main(argv)`, and handles `/help` (prints the slash catalog inline), `/model` (shows current provider/model fallback), `/model list` (lists provider registry status), `/model set <provider> [model]` (persists `ai.provider` / `ai.model` to `<project>/.mythic-vibe.json`), `/knowledge status|sources|search`, `/workspace ...`, `/quit` / `/exit`, EOF, and Ctrl+C. Natural inspection prompts such as "Find the memory system" run a read-only context scan through `mythic_vibe_cli.context.companion`, summarizing repository shape and relevant files. Other natural-language prompts run through the selected provider using the existing routing fallback runtime, with `copy-paste` as the guaranteed fallback, and are appended to `.mythic/memory.sqlite` as task/session memory. Resume prompts such as "What were we doing last time?" render directly from that SQLite memory spine before any provider call. Private-knowledge prompts such as "Search my knowledge database for earlier ideas about Hermes memory" search configured private sources and render summaries before any provider call. Workspace prompts such as "Clone my Hermes fork and make a branch for fixing memory" render a proposed workspace plan without mutating local Git state. Bad shlex-quotes emit a parse error and the loop continues; non-zero exit codes from dispatched commands are surfaced and the loop continues. Plugin-contributed slash commands appear in `/help` output but are not yet dispatched by name (the dispatcher path goes through `app.main`, which only knows sub-commands; plugin-contributed name dispatch is a follow-on slice).
- `mythic-vibe knowledge status|sources|search` is a read-only private knowledge interface. SQLite sources are configured under `knowledge.sources` in JSON config or through `MYTHIC_KNOWLEDGE_SQLITE_PATH`, opened with SQLite `mode=ro`, and searched only through validated table/column identifiers plus parameter-bound `LIKE` clauses. PostgreSQL sources may be listed as configured but are not queried in this stdlib Phase 6 slice.
- `mythic-vibe workspace status|clone|open|branch|track|pr|plan` manages local GitHub working directories. The default workspace root is `~/.mythic-vibe/workspaces/`, overridable through `MYTHIC_WORKSPACE_ROOT` or `--workspace-root`. `workspace status` detects the current repo and branch; `workspace open` registers an existing local repo in the workspace registry; `workspace clone` proposes a clone destination and runs `git clone` only when `--yes` is supplied; `workspace branch` proposes branch creation and runs `git switch -c` only when `--yes` is supplied; `workspace track` records the current or supplied branch in the registry; `workspace pr` prepares a PR draft and writes it only when `--write` is supplied; `workspace plan` turns natural workspace requests into an approval-oriented plan and never mutates files or Git state. JSON output is available on each subcommand.
- `mythic-vibe memory last` renders a deterministic resume answer from `.mythic/memory.sqlite`; JSON output includes the memory snapshot and rendered answer. `mythic-vibe memory spine [--limit N]` initializes the SQLite spine if needed and reports counts by memory kind plus recent entries. Existing `memory list|show|compact|rehydrate` conversation-memory commands remain intact under `mythic/ai/`.
