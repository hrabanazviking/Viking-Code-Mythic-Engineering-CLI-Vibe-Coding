# Documentation Hub (contributor index)

> **PH-23.1 (2026-05-05):** This is the original `docs/INDEX.md`
> contributor-orientation hub, preserved here so the new
> mkdocs-material landing at `docs/index.md` can serve operators
> without losing the contributor-flow content. Both pages are
> wired into the site nav: the new landing under "Home", this
> page under "Governance → Contributor Index".
...
- `docs/security/verifying_artifacts.md` — Sigstore signature + SLSA build provenance verification per channel (PH-21.5 / PH-21.6).
- `docs/security/tag_signing.md` — gitsign-based signed-tag maintainer workflow (PH-21.6).
- `docs/security/sbom.json` — CycloneDX v1.6 dependency manifest. Regenerate via `python scripts/regenerate_sbom.py`.

## Distribution Channels (v1.x)

- `packaging/README.md` — channel inventory, trigger semantics, secrets table, maintainer-repo sync recipes.
- `packaging/WHEELHOUSE.md` — operator guide for offline / air-gapped install.
- `packaging/homebrew/mythic-vibe.rb.template` — Homebrew formula template.
- `packaging/scoop/mythic-vibe.json.template` — Scoop manifest template.
- `packaging/aur/PKGBUILD.template` — AUR PKGBUILD template (PH-21.7).
- `packaging/winget/` — winget v1.6 three-file manifest templates (PH-21.8).
- `packaging/pyinstaller/mythic-vibe.spec` — PyInstaller build spec (PH-21.2).
- `packaging/nuitka/build.py` — Nuitka build driver (PH-21.3).
- `packaging/launcher/` — Rust launcher crate (PH-22.1, foundation).
- `packaging/android/` — Android Gradle/Chaquopy project (PH-22.2, foundation).
- `packaging/wasi/` — WebAssembly experimental runtime (PH-22.3, foundation).
