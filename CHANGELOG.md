...
- Wired `cmd_scan` to emit `before_scan` and `after_scan` through `PluginHookDispatcher` on the real-work path. Dry-run scans skip both hooks. The remaining declared hooks (`before_packet`, `after_packet`, `before_verify`, `after_verify`, `before_reflect`, `after_reflect`) stay wired through the dispatcher contract and will be emitted from their matching commands in subsequent slices.
